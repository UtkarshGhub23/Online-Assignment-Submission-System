...
function seedDatabase(database) {
  ...
  const seed = database.transaction(() => {
    ...
    // Admin
    insertUser.run(
      'Admin User', 'admin@assignsys.com', adminHash, 'admin',
      '+15550100', '123 University Ave, Campus Town', 'System administrator for AssignPro portal.',
      'IT Administration', 'System Operations', 'N/A', 'N/A', 1, 1, 'ADM-001'
    );
    // Teacher
    insertUser.run(
      'Dr. Sarah Johnson', 'teacher@assignsys.com', teacherHash, 'teacher',
      '+15550101', '456 Faculty Housing, Campus Town', 'Professor of Computer Science with 10 years teaching experience.',
      'Computer Science', 'B.Sc. CS', 'N/A', 'N/A', 1, 1, 'TCH-102'
    );
    // Students
    insertUser.run(
    ...
    // 4. Enrollments
    insertEnrollment.run(1, 3); // Alex in CS101
    insertEnrollment.run(2, 3); // Alex in WEB201
    insertEnrollment.run(1, 4); // Jamie in CS101
    insertEnrollment.run(3, 4); // Jamie in DB301
    ...
    insertAssignment.run(
      ...
      null, null, futureDate1, 100, 2, 'published', 'medium', '4 hours', 1, 10, 'pdf,zip'
    );
    ...
    insertAssignment.run(
      ...
      null, null, futureDate2, 150, 2, 'published', 'high', '6 hours', 0, 15, 'zip'
    );
    ...
    insertAssignment.run(
      ...
      null, null, futureDate3, 200, 2, 'published', 'medium', '8 hours', 1, 20, 'zip,rar'
    );
    ...
    insertAssignment.run(
      ...
      null, null, pastDate, 50, 2, 'published', 'low', '2 hours', 0, 5, 'pdf,png'
    );

    // 6. Notifications
    insertNotification.run(3, 'Welcome', 'Welcome to AssignPro. Browse your courses and profile.', 'info', '/dashboard');
    insertNotification.run(3, 'New Assignment', 'Python Programming Lab 1 has been posted in CS101.', 'assignment', '/dashboard/assignments/1');
    insertNotification.run(4, 'Welcome', 'Welcome to AssignPro. Browse your courses and profile.', 'info', '/dashboard');
    insertNotification.run(2, 'Course Enrollee', 'Alex Thompson has enrolled in Introduction to Computer Science.', 'enrollment', '/dashboard/courses/1');

    // 7. Activity Logs
    insertActivity.run(1, 'SYSTEM_STARTUP', 'System settings configured and seeded successfully.');
    insertActivity.run(2, 'COURSE_CREATED', 'Dr. Sarah Johnson created WEB201 course.');
    insertActivity.run(3, 'USER_LOGIN', 'Alex Thompson logged in from Chrome browser.');
  });
